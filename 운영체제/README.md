...
1. Preemptive(선점)      
 - 프로세스가 CPU를 점유하고 있는 동안 인터럽트를 발생시켜 다른 프로세스가 해당 CPU를 강제로 점유할 수 있다      
 - 즉, 프로세스가 정상적으로 수행중인 동안 다른 프로세스가 CPU를 강제로 점유하여 실행할 수 있다      
 
2. Non-Preemptive(비선점)
 - 한 프로세스가 CPU를 점유했으면 I/O나 프로세스가 종료될 때까지 다른 프로세스가 CPU를 점유하지 못하는 것이다      
...
페이지 교체 알고리즘
 ...
 - NUR (Not Used Recently)
    ...
    - 최근에 참조되지 않은 페이지를 교체 대상으로 선정하는 측면에서 LRU와 비슷하지만 교체되는 페이지의 참조 시점이 가장 오래됐다는 것을 보장하지 못함
    - 참조 비트(Reference Bit)와 변형 비트(Modified Bit, Dirty Bit)를 사용
    - 참조 비트
        - 페이지가 참조될 때, 1로 세팅
        - 페이지가 참조되지 않았을 때는 0이 되며 페이지는 교체
    - 변형 비트
        - 페이지 내용이 변경되었을 때, 1로 지정
        - 페이지 내용이 변경되지 않았을 때는 0으로 지정
    - 오래전에 참조 했고, 오래전에 변경(사용)한 페이지를 교체한다
    - 이 알고리즘은 하드웨어의 자원으로 동작하기 때문에 LRU에 비해 교체 페이지의 선정이 훨씬 빠르게 결정
</br>
        
### LRU 구현

 - 더블 링크드리스트를 이용하여 구현한다
 - 헤드에 가까운 데이터일수록 최근에 사용한 데이터, 테일에 가까울수록 오랫동안 사용하지 않은 데이터로 간주하여 새로운 데이터가 삽입될 때 가장 먼저 삭제 되도록 함
 - 해시를 추가적으로 이용하여 구현한다면 Get 연산도 O(1)에 가능하다

# 2022-01-08 ) 앞으로 운영체제 10판 공룡책으로 질문을 주기적으로 추가할 예정입니다.  

## 운영체제란 무엇인가?

 - 정보는 불확실성을 측정해서 수치적으로 계산한 것이다. 이 정보의 최소 단위는 bit이다. 
 - 물리적으로 0 -> 1, 1 -> 0 으로 정보의 상태를 바꿀수 있다. 
 - 컴퓨터라는 것은 이러한 정보를 처리하는 하드웨어. 
 - 컴퓨터 시스템을 운영하는 소프트웨어( running at all times process )다. 
 - 애플리케이션 프로그램에게 시스템 서비스를 제공하는 소프트웨어다.
 - 즉슨, 운영체제는 프로세스를 관리하며 시스템적인 하드웨어 자원을 관리한다.  
