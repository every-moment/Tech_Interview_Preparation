...
### 셸정렬
 ...
 - 삽입정렬의 장점은 살리고 단점은 줄인 방식.   
 - 삽입정렬의 단점: 오름차순 기준으로 타겟 원소가 이전의 원소보다 작을경우 이전 원소들 모두 swap을 한다는 것 ->  이전의 원소를 모두 비교, 교환하는 것이 아니라 일정 간격 주기로 띄엄띄엄 검사하면서 교환하여 타겟 원소의 위치를 대략적으로 잡아주는 방식으로 정렬한다.    
 - 이러한 방식은 삽입정렬의 장점인 거의 정렬 된 상태일 경우 정렬 속도가 빠르다는 이점 또한 가져갈 수 있음.   
 - 정렬 로직
    - 간격(Gap)을 설정한다.   
    - 각 간격별로 분류된 부분 리스트에 대해 삽입 정렬을 한다.   
    - 각 부분 리스트에 대한 정렬이 끝나면 간격을 줄인다    
    - 간격이 1이 될 때까지 반복한다.   
    - 초기 간격에 따라서 퍼포먼스가 달라짐. 
 - 불안정 정렬이고 구현이 어렵다는 단점이 있음.   
 - 최악의 경우에는 O(n^2): 역순으로 정렬되어 있을 경우 삽입정렬의 최악의 경우와 같다.       
 - 최선의 경우에는 O(nlogn): 배열이 이미 정렬되어 있고 gap sequence가 현재 리스트의 크기를 2^n 으로 나눌 수 있을 때 -> 마치 분할정복처럼 동작.       
 - 평균적으로 O(n^1.5)    
</br> 
...
### 계수정렬(Counting Sort)
 ...
 - 크기를 기준으로 개수를 센다.   
 - 시간복잡도: O(n+k): n은 원소 수, k는 원소 중 가장 큰 수의 값.     
...
### 기수정렬(Radix Sort)
 ...
 - n개의 d자리 정수들
 - 가장 낮은 자리수부터 정렬  -> 자리수들을 정렬할때 계수정렬을 씀.   
 - 시간복잡도: O(d(n+k)) -> d가 상수고 O(k)가 O(n)을 만족하면 O(n)이라 볼 수 있음.   
 - 전체 크기에 기수 테이블의 크기만한 메모리가 더 필요하다는 단점이 있음. 또한, 자릿수가 없는 것은 정렬할 수 없음.   
 - 안정정렬.    
</br>
